...
//USCL cube(CUBE_SIZE_4x4x4, RGB_CUBE , D3, D2,  60 , 3, SPI_speed_32); // uncomment this if you are building for Wemos D1 mini
USCL cube(CUBE_SIZE_4x4x4, RGB_CUBE , D3, D2,  60 , 3, SPI_speed_16); // My SEEED cube with Wemos D1 mini
...
// NOTE 2:
// NOTE 3:
//USCL cube(CUBE_SIZE_8x8x8, RGB_CUBE , PA0, PA1,  60 , 4, SPI_speed_8);
//USCL cube(CUBE_SIZE_4x4x4, RGB_CUBE , PA0, PA1,  60 , 7, SPI_speed_4);
//USCL cube(CUBE_SIZE_4x4x4, RGB_CUBE , PA0, PA1,  60 , 7, SPI_speed_16); // My SEEED cube V1
//
/*
...
void plasma6(float time, USCL & cube, uint16_t shiftDelay)
{
  ...
  */
  ...
  float step_offsetV = 720.0;
  float xoffsetV = float(PI / (((step_offsetV) + random(step_offsetV)) / float(cubesize))); //19//24
  float yoffsetV = float(PI / (((step_offsetV) + random(step_offsetV)) / float(cubesize))); //37//37
  float zoffsetV = float(PI / (((step_offsetV) + random(step_offsetV)) / float(cubesize))); //24//19
  ...
  while (millis() - startTime < time)
  {
    ...
    for ( int x = 0; x < cubesize ; x++)
    {
      ...
      fxV = cos(xxV + (xmap[x] / 1000.0));
      for (int   z = 0; z < cubesize; z++)
      {
        fz = sin(zz + (xmap[z]) / 1000.0) ;
        fz = cos(zzV  + (xmap[z]) / 1000.0) ;
        for ( int  y = 0 ; y < cubesize ; y++ )
        {
          fy = cos(yy + (xmap[y] / 1000.0));
          fyV = cos(yyV + (xmap[y] / 1000.0));
          plasma =  (180.0 + 60.0 * (fx + fz + fy) ); // *max=3

          hue = int(plasma ) ;
          V =   0.5 + 0.5 * sin(2.0 * PI * (fxV + fzV +  fyV) ) ;
